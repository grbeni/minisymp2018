...
Programmable controllers are used extensively in several domains such as automotive and railways systems. These controllers react to the stimuli received from their environments in accordance with their internal states. The development of such reactive systems are oftentimes based on statecharts assisted by modeling tools. Several tools support the development of composite systems as the composition of individual statecharts according to different semantics. This paper presents the new composition language of the \gamma\ Statechart Composition Framework, which supports statechart composition according to three different semantics: asynchronous-reactive, synchronous-reactive and cascade. Asynchronous-reactive semantics supports the proper abstraction of distributed communication,
synchronous-reactive supports the modeling of highly synchronous
communication, and cascade is for the linear decomposition of a single function. The introduction of the language elements are supported with simple examples. In the future, we plan to extend the code generation and verification capabilities of the framework to support the new modeling semantics.
\end{abstract}
