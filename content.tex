...
\begin{lstlisting}
// Component instances with atomic component types
...
\begin{itemize}
	\item Capacity specifies the maximum number of messages that can be stored in the particular
	queue.
	\item Priority specifies the order in which the contents of message queues are retrieved during
	the execution of the asynchronous component. A message is always retrieved from a
	non-empty queue with the highest priority. Priority values can be any integers.
	\item Event references specify the types of messages that can be stored in the particular message queue.
\end{itemize}
...
get transmitted.
A synchronous component wrapper also has one or more control specifications, which
...
sync Crossroad [
  ...
  component priorityLight : TrafficLight
  component secondaryLight : TrafficLight
  // Bindings of composite component ports to internal ports
  bind police -> controller.policeInterrupt
  bind priorityLightOutput -> priorityLight.lightCommands
  bind secondaryLightOutput -> secondaryLight.lightCommands
  // Channel definitions connecting internal ports
